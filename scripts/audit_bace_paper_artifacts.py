#!/usr/bin/env python3
"""Audit the four standardized BACE paper artifact roots as one frozen set."""
...
REQUIRED_JSON = ("summary.json", "run_manifest.json", "final_artifact_audit.json")
...
def _write_frozen_csv(
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != text:
            raise FileExistsError(f"Existing combined BACE artifact differs: {path}")
        return
...
def audit_bace_artifacts(
    ...
    *,
    ...
    if not selected_methods:
        raise ValueError("At least one BACE method is required for artifact audit.")
    known = dict(METHODS)
    for slug, display in selected_methods:
        if known.get(slug) != display:
            raise ValueError(f"Unsupported BACE method identity: {slug}/{display}")
    threshold_path = (
        Path(thresholds_path).expanduser().resolve()
        if thresholds_path is not None
        else paper_root / "thresholds.json"
    )
    threshold_contract = load_bace_thresholds(threshold_path)
    ...
    for slug, display in selected_methods:
        ...
        if missing:
            raise FileNotFoundError(f"BACE {display} artifacts are missing/empty: {missing}")
        ...
        if tuple(headers3) != FIGURE3_FIELDS:
            raise ValueError(f"BACE Figure 3 schema mismatch for {display}: {headers3}")
        if tuple(headers4) != FIGURE4_FIELDS:
            raise ValueError(f"BACE Figure 4 schema mismatch for {display}: {headers4}")
        if tuple(headers2) != TABLE2_FIELDS:
            raise ValueError(f"BACE Table 2 schema mismatch for {display}: {headers2}")
        if len(rows3) != 20 or [int(row["k"]) for row in rows3] != list(range(1, 21)):
            raise ValueError(f"BACE Figure 3 K grid is not exactly 1..20 for {display}")
        if any(row["method"] != display for row in rows3 + rows4 + rows2):
            raise ValueError(f"BACE method label changed within {method_root}")
        coverages3 = [
            _finite_rate(row["coverage"], field="coverage", path=figure3_path)
            for row in rows3
        ]
        if any(b + 1e-12 < a for a, b in zip(coverages3, coverages3[1:])):
            raise ValueError(f"BACE Figure 3 coverage is not monotone for {display}")
        for row in rows3:
        ...
        if not _same_grid(grid, expected_thresholds):
            raise ValueError(f"BACE Figure 4 threshold protocol differs for {display}")
        coverages4 = [
            _finite_rate(row["coverage"], field="coverage", path=figure4_path)
            for row in rows4
        ]
        if any(b + 1e-12 < a for a, b in zip(coverages4, coverages4[1:])):
            raise ValueError(f"BACE Figure 4 coverage is not monotone for {display}")
        if len(rows2) != 1 or int(rows2[0]["k"]) != 10:
            raise ValueError(f"BACE Table 2 must contain one K=10 row for {display}")
        ...
        if not math.isclose(
            ...
            raise ValueError(f"BACE Table 2/Figure 3 K=10 coverage differs for {display}")
        ...
        if (table_cost is None) != (figure3_k10_cost is None) or (
            ...
            raise ValueError(f"BACE Table 2/Figure 3 K=10 cost differs for {display}")
        ...
        if summary.get("dataset") != "BACE" or summary.get("method") != display:
            raise ValueError(f"BACE summary identity mismatch for {display}")
        if summary.get("distance_line") != DISTANCE_LINE or summary.get("cf_mode") != CF_MODE:
            raise ValueError(f"BACE distance/strict-flip semantics changed for {display}")
        if manifest.get("candidate_set_preselected") is not True:
            raise ValueError(f"BACE candidates are not marked preselected for {display}")
        if manifest.get("selection_performed_in_eval") is not False:
            raise ValueError(f"BACE evaluator-side selection detected for {display}")
        if final_audit.get("passed") is not True:
            raise ValueError(f"BACE final artifact audit is not passing for {display}")
        ...
        if len(parent_ids_sha256) != 64:
            raise ValueError(f"BACE parent ID hash is missing for {display}")
        if reference_parent_count is None:
            ...
            raise ValueError(
                f"BACE parent universe differs: {display}={parent_count}, "
                f"reference={reference_parent_count}"
            )
        elif parent_ids_sha256 != reference_parent_ids_sha256:
            raise ValueError(f"BACE parent ID universe/order differs for {display}")
        ...
        if not teacher_path or not molclr_checkpoint:
            raise ValueError(f"BACE teacher/MolCLR lineage is missing for {display}")
        if reference_teacher_path is None:
            ...
            raise ValueError(f"BACE teacher/MolCLR lineage differs for {display}")
    ...
    combined_paths = {
        ...
        "table2": paper_root / "table2_bace_k10.csv",
    }
    ...
    return {
        "schema_version": "bace_common4_audit_v1",
        "passed": True,
        "dataset": "BACE",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--root",
        default="outputs/hpc/eval/paper/bace_common3_standardized_v1",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if unknown:
        raise ValueError(f"Unknown BACE method slugs: {unknown}")
    ...
    output = (
        ...
        if args.output_json
        else Path(args.root).expanduser().resolve() / "bace_paper_artifact_audit.json"
    )
    output.write_text(
    ...
    print("[BACE_PAPER_ARTIFACT_AUDIT_PASS]", flush=True)
    return 0
