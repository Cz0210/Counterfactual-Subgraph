...
OUT_ROOT=outputs/hpc/selectors/widegrid_gt_fullgraph_embedding_label1_relaxed
...
for RUN_ID in "${RUN_IDS[@]}"; do
  ...
  python "${DIAGNOSE}" \
    ...
    --label 1 \
    --embedding-field final_fragment_embedding \
    --out-json "${DIAG_DIR}/widegrid_candidate_pool_with_embeddings_diagnosis.json" \
    --out-txt "${DIAG_DIR}/widegrid_candidate_pool_with_embeddings_diagnosis.txt"

  for BETA in "${BETA_COVERAGE_LIST[@]}"; do
    BETA_TAG=${BETA//./p}
    for GAMMA in "${GAMMA_REDUNDANCY_LIST[@]}"; do
      GAMMA_TAG=${GAMMA//./p}
      OUT_DIR="${OUT_ROOT}/label1_${RUN_ID}/beta_${BETA_TAG}_gamma_${GAMMA_TAG}"
      mkdir -p "${OUT_DIR}"
      echo "===== SELECT RELAXED GT label1_${RUN_ID} beta=${BETA} gamma=${GAMMA} ====="
      ...
      echo "===== SUMMARY RELAXED GT label1_${RUN_ID} beta=${BETA} gamma=${GAMMA} ====="
      cat "${OUT_DIR}/selector_summary.json"
    done
  done
done

echo "===== ALL RELAXED GT EMBEDDING WIDEGRID SELECTOR RUNS DONE ====="
