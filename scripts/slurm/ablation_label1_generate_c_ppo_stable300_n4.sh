...
for p in "${DATASET_PATH}" "${BASE_MODEL_PATH}" "${SFT_LORA_PATH}" "${PPO_CHECKPOINT_PATH}" "${TEACHER_PATH}"; do
  if [ ! -e "${p}" ]; then
...
if [ "${FORCE_REGEN}" = "true" ] || [ "${POOL_NONEMPTY}" != "true" ]; then
  echo "===== GENERATING ABLATION POOL: C SFT+PPO stable300 ====="
