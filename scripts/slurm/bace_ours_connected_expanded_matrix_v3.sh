#!/bin/bash
#SBATCH --job-name=bace_ours_conn_matrix_x_v3
...
OUTPUT_DIR=${OUTPUT_DIR:-$ARTIFACT_ROOT/outputs/hpc/optimization/bace_ours_connected_residual_v3_expanded/calibration_matrix}
...
for path in "$CANDIDATE_POOL" "$CALIBRATION_CSV" "$TEACHER_PATH" "$MOLCLR_CHECKPOINT"; do
  test -s "$path" || { echo "[BACE_CONNECTED_EXPANDED_MATRIX_CONFIG_ERROR] missing $path" >&2; exit 2; }
done
[[ "$CALIBRATION_CSV" != *test* ]] || { echo "[BACE_CONNECTED_EXPANDED_MATRIX_LEAKAGE] test input" >&2; exit 2; }
args=(
  ...
  --expected-parent-count 60 --cf-mode strict_flip
...
if [ "$DRY_RUN" = 1 ] || [ "$VALIDATE_ONLY" = 1 ]; then
  echo "[BACE_CONNECTED_EXPANDED_MATRIX_V3_VALIDATE_OK]"
  exit 0
fi
"${args[@]}"
python scripts/audit_wnode_action_matrix.py \
  --config configs/hpc.yaml --run-dir "$OUTPUT_DIR" \
  --expected-parent-count 60 --require-strict-flip-pair
python - "$OUTPUT_DIR/matrix_audit.json" <<'PY'
import json, sys
p = json.load(open(sys.argv[1]))
assert p["audit_passed"] is True
...
PY
echo "[BACE_CONNECTED_EXPANDED_MATRIX_V3_SUCCESS]"
