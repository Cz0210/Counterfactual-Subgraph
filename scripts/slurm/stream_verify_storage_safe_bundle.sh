#!/usr/bin/env bash
# CPU-only override: stream verification extracts no result and needs no GPU.
#SBATCH --job-name=t8-stream-verify
...
python scripts/hpc/t8/stream_verify_storage_safe_bundle.py --config configs/hpc.yaml "$@"
