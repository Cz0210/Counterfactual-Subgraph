#!/usr/bin/env bash
# CPU-only override: exact T8 merge/package needs no GPU and must use local scratch.
#SBATCH --job-name=t8-storage-safe-package
...
python scripts/hpc/t8/run_storage_safe_merge_package.py --config configs/hpc.yaml "$@"
