...
OUT_ROOT=outputs/hpc/selectors/param_sweep_gt_fullgraph_embedding_cov20
...
for path in "${SELECTOR}" "${CHECKER}"; do
  if [ ! -f "${path}" ]; then
...
for RUN_ID in "${RUN_IDS[@]}"; do
  ...
  if [ ! -f "${GT_POOL}" ]; then
    ...
    echo "[HINT] Run sbatch scripts/slurm/add_embeddings_camc_gt_fullgraph_label1_clean.sh first."
    exit 1
  fi
  ...
  for GAMMA in "${GAMMAS[@]}"; do
    ...
    mkdir -p "${OUT_DIR}"
    echo "===== SELECT GT label1_${RUN_ID} gamma=${GAMMA} ====="
    ...
    echo "===== SUMMARY GT label1_${RUN_ID} gamma=${GAMMA} ====="
    cat "${OUT_DIR}/selector_summary.json"
  done
done

echo "===== ALL GT EMBEDDING SELECTOR SWEEPS DONE ====="
