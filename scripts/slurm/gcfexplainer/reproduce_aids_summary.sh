...
echo "===== GCFEXPLAINER OFFICIAL AIDS SUMMARY ENV CHECK ====="
...
if [ ! -f "${OFFICIAL_DIR}/results/aids/runs/counterfactuals.pt" ]; then
  echo "[WARN] expected official candidate file is missing before summary: ${OFFICIAL_DIR}/results/aids/runs/counterfactuals.pt"
  echo "[WARN] Run reproduce_aids_vrrw.sh first, or use reproduce_aids_all.sh, if summary.py fails."
fi
...
if [ -d "${OFFICIAL_DIR}/results/aids" ]; then
  ...
  echo "[WARN] official results directory not found after summary: ${OFFICIAL_DIR}/results/aids"
fi

echo "===== GCFEXPLAINER AIDS SUMMARY DONE ====="
