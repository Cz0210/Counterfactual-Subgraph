...
for p in "${DATASET_PATH}" "${BASE_MODEL_PATH}" "${TEACHER_PATH}"; do
  if [ ! -e "${p}" ]; then
...
if [ "${FORCE_REGEN}" = "true" ] || [ "${POOL_NONEMPTY}" != "true" ]; then
  echo "===== GENERATING ABLATION POOL: A BASE ChemLLM ====="
