#!/bin/bash
#SBATCH --job-name=bace_ours_conn_matrix_r1_v4
...
POOL_ROOT=${POOL_ROOT:-$ARTIFACT_ROOT/outputs/hpc/candidate_pools/bace_ours_connected_candidateaware_v4/round1_merged}
...
OUTPUT_DIR=${OUTPUT_DIR:-$ARTIFACT_ROOT/outputs/hpc/optimization/bace_ours_connected_candidateaware_v4/round1_calibration_matrix}
...
for path in "$CANDIDATE_POOL" "$POOL_ROOT/candidate_pool_audit.json" "$CALIBRATION_CSV" "$TEACHER_PATH" "$MOLCLR_CHECKPOINT" "$DIAGNOSTIC_THRESHOLDS"; do
  test -s "$path" || { echo "[BACE_CONNECTED_R1_MATRIX_CONFIG_ERROR] missing $path" >&2; exit 2; }
done
readarray -t EXPECTED < <(python - "$POOL_ROOT/candidate_pool_audit.json" <<'PY'
import json, sys
p=json.load(open(sys.argv[1], encoding="utf-8"))
print(p["candidate_count"]); print(p["parent_count"])
PY
)
args=(python scripts/precompute_wnode_action_matrix.py
  ...
  --expected-parent-count 60 --expected-pool-rows "${EXPECTED[0]}"
  --expected-source-parent-count "${EXPECTED[1]}"
  --candidate-universe-policy connected_feasible_v4
  --min-source-atom-ratio 0.0 --max-source-atom-ratio 0.85
  --require-candidate-lineage --cf-mode strict_flip
...
if [ "$DRY_RUN" = 1 ] || [ "$VALIDATE_ONLY" = 1 ]; then
  echo "[BACE_OURS_CONNECTED_R1_MATRIX_V4_VALIDATE_OK]"
  exit 0
fi
test ! -e "$OUTPUT_DIR" || { echo "[BACE_CONNECTED_R1_MATRIX_COLLISION] $OUTPUT_DIR" >&2; exit 2; }
"${args[@]}"
python scripts/audit_wnode_action_matrix.py --config configs/hpc.yaml \
  --run-dir "$OUTPUT_DIR" --expected-parent-count 60 --require-strict-flip-pair
...
echo '[BACE_OURS_CONNECTED_R1_MATRIX_V4_SUCCESS]'
