...
if [ -z "${TEACHER_PATH:-}" ] || [ ! -f "${TEACHER_PATH}" ]; then
  ...
  echo "TEACHER_PATH=/path/to/aids_rf_model.pkl sbatch scripts/slurm/molclr_hiv_eval_ccrcov_full.sh"
  exit 2
fi
...
python scripts/evaluate_ccrcov_with_molclr.py \
  ...
  --embedding-dir "${EMBEDDING_DIR:-outputs/hpc/molclr_ccrcov_embeddings}" \
  --label "${LABEL:-1}" \
  ...
  --output-root "${OUTPUT_ROOT:-outputs/hpc/eval/ccrcov_molclr_hiv_full}" \
  --partial-every "${PARTIAL_EVERY:-5000}"

echo "===== MOLCLR CCRCov FULL DONE ====="
cat "${OUTPUT_ROOT:-outputs/hpc/eval/ccrcov_molclr_hiv_full}/combined/combined_threshold_summary.csv"
