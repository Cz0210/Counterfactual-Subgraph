...
if [ -z "${TEACHER_PATH:-}" ] || [ ! -f "${TEACHER_PATH}" ]; then
  ...
  echo "TEACHER_PATH=/path/to/aids_rf_model.pkl sbatch scripts/slurm/greed_hiv_eval_ccrcov_full.sh"
  exit 2
fi

echo "===== GREED CCRCov FULL ====="
echo "host: $(hostname)"
echo "pwd: $(pwd)"
