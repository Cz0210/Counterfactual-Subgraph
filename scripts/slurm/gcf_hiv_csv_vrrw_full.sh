...
GNN_DIR=${GNN_DIR:-outputs/hpc/gcfexplainer_hiv_csv/gnn}
...
python scripts/gcf_hiv_csv_run_vrrw.py \
  ...
  --sample-size "${GCF_SAMPLE_SIZE:-10000}" \
  --target-label "${TARGET_LABEL}" \
  --device "${DEVICE:-cuda}" \
  --seed "${SEED:-0}"

echo "===== GCF HIV CSV VRRW FULL DONE ====="
