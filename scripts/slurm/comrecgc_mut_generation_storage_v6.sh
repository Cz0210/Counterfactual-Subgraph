#!/bin/bash
#SBATCH --job-name=comrecgc_mut_retry8_storage_v6
...
mkdir -p logs
BASE_ROOT=${BASE_ROOT:-$ARTIFACT_ROOT/outputs/hpc/baselines/comrecgc/mutagenicity/project_full_retry8_storage_v6}
PERSISTENT_SCRATCH_ROOT=${PERSISTENT_SCRATCH_ROOT:-/share/project/p20526/u20526/counterfactual-subgraph}
SCRATCH_STATE_ROOT=${SCRATCH_STATE_ROOT:-$PERSISTENT_SCRATCH_ROOT/comrecgc_mut_retry8_v6}
...
DATASET_DIR=${DATASET_DIR:-$ARTIFACT_ROOT/outputs/hpc/mutagenicity/baselines/gcfexplainer/smoke_v1/dataset}
GNN_CHECKPOINT=${GNN_CHECKPOINT:-$ARTIFACT_ROOT/outputs/hpc/mutagenicity/baselines/gcfexplainer/full_v2/gnn/model_best.pth}
DISTANCE_CHECKPOINT=${DISTANCE_CHECKPOINT:-$ARTIFACT_ROOT/outputs/hpc/pretrained/gcfexplainer/mutagenicity/neurosed/best_model.pt}
...
for path in "$DATASET_DIR" "$GNN_CHECKPOINT" "$DISTANCE_CHECKPOINT"; do test -e "$path" || { echo "missing input: $path" >&2; exit 2; }; done
...
args=(python scripts/baselines/comrecgc/run_generation.py
  --config configs/hpc.yaml --set inference.fallback_to_heuristic=false
  --route project --dataset mutagenicity --mode full
  --project-root "$PROJECT_ROOT" --upstream-root "$COMRECGC_ROOT"
  --dataset-dir "$DATASET_DIR" --gnn-checkpoint "$GNN_CHECKPOINT"
...
if [[ "$DRY_RUN" == 1 || "$VALIDATE_ONLY" == 1 ]]; then echo '[COMRECGC_MUT_STORAGE_V6_VALIDATE_OK]'; exit 0; fi
...
elif [[ -e "$BASE_ROOT/generation" ]]; then
  echo "generation path collision: $BASE_ROOT/generation" >&2; exit 2
else
  ln -s "$OUTPUT_DIR" "$BASE_ROOT/generation"
fi
"${args[@]}"
test -s "$OUTPUT_DIR/_RUN_COMPLETE.json"
test -s "$GRAPH_STATE_DIR/authoritative_graph_store.sqlite3"
echo '[COMRECGC_MUT_STORAGE_V6_SUCCESS]'
