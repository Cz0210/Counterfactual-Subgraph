#!/bin/bash
#SBATCH -J full_pool_stable300_n4
...
OUT_DIR=/share/home/u20526/czx/counterfactual-subgraph/outputs/hpc/full_candidate_pools/stable300_label1_n4
POOL=${OUT_DIR}/candidate_pool.jsonl
OUT_SUMMARY=${OUT_DIR}/generation_summary.json

AUDIT_DIR=/share/home/u20526/czx/counterfactual-subgraph/outputs/hpc/audits/stable300_label1_n4_full_pool_audit
...
if [ -f scripts/generate_full_candidate_pool_stable.py ]; then
  GENERATE_SCRIPT=scripts/generate_full_candidate_pool_stable.py
elif [ -f scripts/generate_full_candidate_pool.py ]; then
  GENERATE_SCRIPT=scripts/generate_full_candidate_pool.py
elif [ -f scripts/generate_candidate_pool.py ]; then
  GENERATE_SCRIPT=scripts/generate_candidate_pool.py
else
  echo "[ERROR] no candidate pool generation script found"
  exit 1
fi
...
if [ "${FORCE_REGEN}" = "true" ] || [ "${POOL_NONEMPTY}" != "true" ]; then
  echo "===== GENERATING FULL CANDIDATE POOL ====="
