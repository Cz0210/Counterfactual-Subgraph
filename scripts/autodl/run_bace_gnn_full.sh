...
autodl_require_file "$DATASET_CONFIG"
autodl_require_file "$AUTODL_CONFIG"
autodl_require_dir "$BACE_SPLIT_ROOT"

GPU_LINE="$(autodl_select_one_gpu)" || exit $?
IFS=$'\t' read -r GPU_INDEX GPU_UUID <<< "$GPU_LINE"
OUTPUT_DIR="${BACE_GNN_FULL_OUTPUT:-$(autodl_new_output_dir bace "$PRIMARY_GNN_BACKBONE" full)}"
INPUT_ARGS=()
INPUT_MANIFEST="$(autodl_find_split_manifest "$BACE_SPLIT_ROOT" || true)"
...
exec "$AUTODL_PYTHON" "$SCRIPT_DIR/exp_run.py" \
  ...
  --required-log-marker "[BACE_GNN_TRAIN_PASS]" \
  -- \
