...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=_env_float("SUBSTRUCTURE_DISTANCE_MIN_ATOM_RATIO", 0.10),
        help="Minimum candidate atom_count / parent_atom_count for nearest-parent-subgraph retrieval.",
    )
    parser.add_argument(
        "--substructure-distance-max-atom-ratio",
        type=float,
        default=_env_float("SUBSTRUCTURE_DISTANCE_MAX_ATOM_RATIO", 0.65),
        help="Maximum candidate atom_count / parent_atom_count for nearest-parent-subgraph retrieval.",
    )
    parser.add_argument(
        "--substructure-distance-topk",
        type=int,
        default=_env_int("SUBSTRUCTURE_DISTANCE_TOPK", 20),
        help="Fingerprint-ranked top-k candidates that receive RDKit MCS scoring.",
    )
    parser.add_argument(
        "--substructure-distance-mcs-timeout",
        type=int,
        default=_env_int("SUBSTRUCTURE_DISTANCE_MCS_TIMEOUT", 1),
        help="RDKit MCS timeout in seconds for nearest-parent-subgraph similarity.",
    )
    parser.add_argument(
        "--substructure-distance-sim-threshold",
        type=float,
        default=_env_float("SUBSTRUCTURE_DISTANCE_SIM_THRESHOLD", 0.0),
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Hard-fail strict/projected fragments below this atom count; 0 disables the guard.",
    )
    parser.add_argument(
        "--tiny-fragment-hard-fail-penalty",
        type=float,
        default=-6.0,
        help="Reward total used when --min-fragment-atoms hard-fails a fragment.",
    )
    parser.add_argument(
        "--enable-size-window-reward",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Apply a soft reward/penalty based on final fragment atom ratio.",
    )
    parser.add_argument(
        "--size-window-low",
        type=float,
        default=0.15,
        help="Lower inclusive bound for the preferred final fragment atom ratio.",
    )
    parser.add_argument(
        "--size-window-high",
        type=float,
        default=0.65,
        help="Upper inclusive bound for the preferred final fragment atom ratio.",
    )
    parser.add_argument(
        "--size-window-bonus",
        type=float,
        default=0.4,
        help="Soft bonus for final fragments whose atom ratio is inside the preferred window.",
    )
    parser.add_argument(
        "--size-window-small-penalty",
        type=float,
        default=-0.4,
        help="Soft penalty for final fragments whose atom ratio falls below the preferred window.",
    )
    parser.add_argument(
        "--size-window-large-penalty",
        type=float,
        default=-0.4,
        help="Soft penalty for final fragments whose atom ratio exceeds the preferred window without triggering hard guards.",
    )
    parser.add_argument(
...
def run_decoded_chem_ppo_loop(
    *,
    ...
    for step_index in range(1, int(args.max_steps) + 1):
        ...
        tiny_fragment_hard_fail_count = sum(
            1 for reward_log in reward_logs if reward_log.get("tiny_fragment_hard_fail")
        )
        logger.info(
            "[CHEM_REWARD_REPAIR_STATS] repair_attempted=%s repair_success=%s component_salvage_attempted=%s component_salvage_success=%s near_parent_hard_fail=%s tiny_fragment_hard_fail=%s",
            repair_attempted_count,
            repair_success_count,
            component_salvage_attempted_count,
            component_salvage_success_count,
            near_parent_hard_fail_count,
            tiny_fragment_hard_fail_count,
        )
        ...
        for reward_log in reward_logs_to_print:
            ...
            if reward_log.get("failure_tag"):
                ...
                "[CHEM_REWARD_FAILURE] id=%s failure_tag=%s parse_failed_reason=%s parse_stage=%s raw_has_dummy=%s raw_dummy_count=%s parsed_raw_with_dummy=%s parsed_core=%s dummy_removed_before_parse=%s invalid_detail=%s fragment_chars=%s repair_attempted=%s repair_success=%s repair_method=%s repair_reason=%s repair_edit_distance=%s repair_suffix_trim_count=%s repair_added_parentheses=%s repair_added_ring_closures=%s repaired_raw_fragment=%s repaired_fragment_chars=%s repaired_parse_stage=%s repaired_parsed_raw=%s repaired_parsed_core=%s repair_failure_reason=%s repair_failure_stage=%s repair_candidate_count=%s repair_candidates_parse_ok=%s repair_candidates_core_ok=%s repair_candidates_parent_ok=%s repair_candidates_projection_ok=%s repair_best_candidate=%s repair_accept_stage=%s repair_candidate_accepted=%s repair_candidate_rejected_reason=%s component_salvage_attempted=%s component_salvage_success=%s component_count=%s raw_component_count=%s core_component_count=%s salvage_method=%s salvaged_fragment=%s salvaged_atom_count=%s component_salvage_failure_reason=%s component_salvage_stage=%s component_salvage_candidate_count=%s component_salvage_best_candidate=%s multi_dummy_hard_fail=%s dummy_salvage_attempted=%s dummy_salvage_success=%s dummy_salvage_method=%s dummy_salvaged_fragment=%s near_parent_hard_fail=%s tiny_fragment_hard_fail=%s fragment_atom_count=%s min_fragment_atoms=%s residual_atom_count=%s residual_atom_ratio=%s projection_attempted=%s projection_success=%s projection_method=%s projection_score=%s projection_source=%s projected_fragment=%s projection_atom_count=%s projection_atom_ratio=%s projection_penalty=%s projection_penalty_config=%s projection_penalty_applied=%s reward_before_projection_penalty=%s reward_after_projection_penalty=%s num_projection_candidates=%s projection_reason=%s size_window_reward=%s size_window_bucket=%s size_window_low=%s size_window_high=%s final_fragment_atom_count=%s final_fragment_atom_ratio=%s error=%s",
                    ...
                    reward_log.get("repair_failure_reason"),
                    reward_log.get("repair_failure_stage"),
                    reward_log.get("repair_candidate_count"),
                    reward_log.get("repair_candidates_parse_ok"),
                    reward_log.get("repair_candidates_core_ok"),
                    reward_log.get("repair_candidates_parent_ok"),
                    reward_log.get("repair_candidates_projection_ok"),
                    reward_log.get("repair_best_candidate"),
                    reward_log.get("repair_accept_stage"),
                    reward_log.get("repair_candidate_accepted"),
                    reward_log.get("repair_candidate_rejected_reason"),
                    reward_log.get("component_salvage_attempted"),
                    reward_log.get("component_salvage_success"),
                    reward_log.get("component_count"),
                    reward_log.get("raw_component_count"),
                    reward_log.get("core_component_count"),
                    reward_log.get("salvage_method"),
                    reward_log.get("salvaged_fragment"),
                    reward_log.get("salvaged_atom_count"),
                    reward_log.get("component_salvage_failure_reason"),
                    reward_log.get("component_salvage_stage"),
                    reward_log.get("component_salvage_candidate_count"),
                    reward_log.get("component_salvage_best_candidate"),
                    ...
                    reward_log.get("tiny_fragment_hard_fail"),
                    reward_log.get("fragment_atom_count"),
                    reward_log.get("min_fragment_atoms"),
                    ...
                    reward_log.get("size_window_bucket"),
                    reward_log.get("size_window_low"),
                    reward_log.get("size_window_high"),
                    reward_log.get("final_fragment_atom_count"),
                    reward_log.get("final_fragment_atom_ratio"),
            ...
            logger.info(
                ...
                reward_log.get("num_projection_candidates"),
                reward_log.get("nearest_parent_subgraph_smiles"),
...
def main() -> None:
    ...
    logger.info(
        "[COMPONENT_DUMMY_RESIDUAL_GUARD_CONFIG] component_salvage=%s component_method=%s component_min_atoms=%s multi_dummy_threshold=%s light_dummy_salvage=%s near_parent_ratio=%s min_residual_atoms=%s min_residual_ratio=%s min_fragment_atoms=%s tiny_fragment_penalty=%s size_window_enabled=%s size_window_low=%s size_window_high=%s size_window_bonus=%s size_window_small_penalty=%s size_window_large_penalty=%s",
        ...
        args.min_fragment_atoms,
        args.tiny_fragment_hard_fail_penalty,
        args.enable_size_window_reward,
        args.size_window_low,
        args.size_window_high,
        args.size_window_bonus,
