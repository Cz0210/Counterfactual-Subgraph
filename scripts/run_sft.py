"""Config-driven SFT runtime entrypoint for local and HPC use."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--stage",
        choices=[TrainStage.FORMAT_SFT.value, TrainStage.WEAK_SUPERVISION_SFT.value],
        help="Override training.stage for SFT.",
    )
...
def main() -> None:
    ...
    prepared = prepare_runtime(args, stage_name="sft", stage_config_name="sft.yaml")

    stage_value = args.stage or prepared.config.get("training", {}).get(
        "stage",
        TrainStage.WEAK_SUPERVISION_SFT.value,
    )
    max_steps = args.max_steps or int(prepared.config.get("training", {}).get("max_steps", 0))
    request = TrainingRunRequest(
        stage=TrainStage(stage_value),
    ...
    prepared.logger.info(
        "Prepared SFT request: stage=%s max_steps=%s output_dir=%s",
        request.stage.value,
        request.max_steps,
        request.output_dir,
    )
    prepared.logger.warning("SFT training loop is not implemented yet; runtime preparation only.")
