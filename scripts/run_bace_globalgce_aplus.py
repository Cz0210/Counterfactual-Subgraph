#!/usr/bin/env python3
"""BACE GIN A+ isolated CLI; train/validation only, no matrix writer."""
...
def main():
    ...
    p.add_argument('--action', choices=('prepare', 'train', 'train-canary', 'export', 'status', 'owner'), required=True)
    ...
    if args.action == 'prepare' and os.environ.get('CUDA_VISIBLE_DEVICES') not in ('', '-1'):
        p.error('CPU input preparation requires CUDA_VISIBLE_DEVICES empty')
    ...
    if args.action == 'prepare':
        from src.baselines.bace_globalgce_aplus_inputs import prepare
        result = prepare(args.repair_config, args.output_root)
        ...
        from src.baselines.bace_globalgce_aplus_training import run_training
