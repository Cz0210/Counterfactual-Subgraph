#!/usr/bin/env bash
# CPU-only group array for the already completed exact T8 mining shards.
...
case "$job_tmp_base" in /share/*|/ssdfs/*) echo "group scratch is not node-local" >&2; exit 67;; esac
job_tmp="$(mktemp -d "$job_tmp_base/t8-hier-group-${SLURM_ARRAY_JOB_ID}-${SLURM_ARRAY_TASK_ID}.XXXXXX")"
child_pid=""
...
printf -v group_name 'group-%02d' "$SLURM_ARRAY_TASK_ID"
group_root="$T8_HIERARCHICAL_GROUPS_ROOT/$group_name"
mkdir -p "$T8_HIERARCHICAL_GROUPS_ROOT"
echo "python=$T8_PYTHON"
"$T8_PYTHON" --version
echo "cuda_visible_devices=${CUDA_VISIBLE_DEVICES:-}"
echo "execution_commit=$T8_EXPECTED_COMMIT"
echo "group_index=$SLURM_ARRAY_TASK_ID"

"$T8_PYTHON" scripts/hpc/t8/run_hierarchical_group_merge.py \
...
wait "$child_pid"
child_pid=""
echo "T8_HPC_HIERARCHICAL_GROUP_PASS group=$SLURM_ARRAY_TASK_ID root=$group_root"
