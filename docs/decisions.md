...
Taste smoke and replay stages already expose valid stage-specific markers,
but none is a paper cell.  The generic registry validates standardized shape
and shared oracle fields but does not by itself reopen the managed T3
temperature publication or distinguish every T11--T14 terminal schema.

### Decision

Add one dataset-specific, read-only matrix appender.  It accepts only exact
full-stage schemas and PASS bytes, reopens all already-passing Taste cells on
every append, proves a common held T3 GINE/temperature plus split, test-parent,
MolCLR and threshold identities, and requires independent freeze/test replay.
Preserve every non-target row exactly and publish a new hash-closed authority
through a no-replace atomic rename.  Keep the scoped private-research policy
explicit and do not infer an upstream licence.  Reserve an exact future T12
terminal contract so its current replay canary remains ineligible.

### Consequences

- T11, T13 and T14 can be appended independently as their real final roots
  arrive; T12 remains fail-closed until its production terminal exists.
- A marker, smoke root, science-only root, mismatched temperature or test-side
  selection cannot advance the matrix count.
- The existing continuation queue can call the CLI after each PASS; no new
  controller or scientific recomputation is introduced.
- The BACE frozen-standardization CLI now exposes the already-supported
  `GlobalGCE` method instead of rejecting the task generated by its builder.

---
## [2026-09-01] Close T12 with a generation-gated full-graph paper stage

### Motivation

T12's exact 10k-to-20k generation and lossless native candidate snapshot are
now independently verifiable, but that train-side marker is not a paper-cell
result.  The remaining work must order candidates without test leakage and
must evaluate the official full graphs rather than reinterpret them as
deletions or transformation rules.

### Decision

Add one TasteMolNet-specific paper stage whose only generation input is the
exact `[TASTE_T12_GCF_GENERATION_PASS]` receipt.  Reopen the native snapshot,
replay official `importance_parts[0] >= 0.5` ordering up to the 3,778-source
limit, and decode each graph with its source-lineage atom/bond sidecars.  Use
the same frozen T3 GINE to retain non-Sweet full graphs.  Evaluate the complete
calibration Cartesian product with direct parent-to-full-graph MolCLR WNode,
freeze the greedy top-20 order durably, and only then open held-out test.

The separate verifier re-derives the candidate surface, replays chunk hashes,
the calibration selector and standardized CSV bytes, runs the 4-by-4 registry
gate, and exclusively writes `[TASTE_GCF_PASS]`.  Its public run and audit
schemas are `tastemolnet_t12_final_run_manifest_v1` and
`tastemolnet_t12_terminal_verification_v1`.  A narrow follower sidecar joins
the already persistent generation queue to this paper stage and emits the
matrix root locator only after exact terminal PASS.

### Consequences

- Test cannot influence candidate materialization, threshold selection or
  ordering; the first test-access receipt is immutable across resume.
- GCF remains a nearest full-counterfactual-graph method.  No residual,
  deletion match, RF oracle, or rule-application context is introduced.
- Calibration and test are parent-chunked with exact identity/hash replay, so
  a long WNode Cartesian evaluation can resume without changing results.
- Figure 3, Figure 4 and Table 2 K=10 use the shared frozen Taste WNode
  threshold authority and the same T3 GINE/MolCLR identities as other cells.

### Status

Implemented and focused-tested locally; deployment must use a fresh paper and
terminal-verification root after the real generation marker appears.
