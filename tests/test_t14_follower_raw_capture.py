...
def test_full_fields_survive_more_than_32_values(tmp_path):
    ...
    assert row['head']==0 and row['lead_head']==3
    assert row['difference_bytes']==row['arrays']['difference']['values'].tobytes()
...
def test_preconsumption_copy_survives_native_array_mutation():
    def move_from_known_graph():pass
    native=np.argmin
    calls=[]
    def consume(value):
        result=native(value);calls.append(1);value[:]=7
        return result
    def move_to_next_graph():
        k=0;i=11;select=3;target_graphs_hashes=[3,4]
        difference=np.array([.1,.2],dtype=np.float64)
        recourse=np.zeros(2);matching_recourses=np.zeros((2,2));target_graphs_embedding=np.zeros((2,2))
        start_embedding=np.zeros(2);selected_elements=np.ones(2);start_elements=1
        target_graphs_importance_parts=np.ones((2,3))
        return proxy.argmin(difference)
    proxy=SimpleNamespace(argmin=consume)
    module=SimpleNamespace(move_from_known_graph=move_from_known_graph,move_to_next_graph=move_to_next_graph,np=proxy)
    with SamplingObserver(module) as observer:
        observer.capture_follower=True;assert move_to_next_graph()==0
    assert calls==[1]
    assert observer.events[0]['arrays']['difference']['values'].tolist()==[.1,.2]
    assert proxy.argmin is consume
