...
def test_owner_queue_child_and_watchdog_are_wired_to_policy():
    root = Path(__file__).resolve().parents[2]
    owner = (root / "src/ablations/llm/existing_gpu_owner.py").read_text()
    queue = (root / "scripts/autodl/gpu_lock.py").read_text()
    child = (root / "scripts/ablations/llm/run_bace_common_downstream.py").read_text()
    assert 'stage_id="llm_gpu_generation"' in owner
    assert 'stage_id="llm_cpu_evaluation"' in queue and 'stage_id="llm_cpu_evaluation"' in child
    assert 'pause or evidence.get("pause_requested")' in owner
    assert "min(60, args.refresh_seconds)" in queue
