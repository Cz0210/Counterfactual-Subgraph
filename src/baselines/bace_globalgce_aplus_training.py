...
from src.baselines.bace_globalgce_aplus import joint_states, materialize
from src.baselines.bace_globalgce_aplus_inputs import setup, read_json, predict_smiles
from src.baselines.globalgce_bace_native_rules import validate_official_globalgce_root
from src.baselines.bace_globalgce_aplus import build_parent as build_parent_native_tensors
...
TRAINING_CONTRACT = {
    'schema_version': 'bace_gin_aplus_epoch35_warmstart_objective_v1',
    'initialization': 'paused_GINE_epoch35_generator_weights_only_new_GIN_optimizer',
    ...
    'repair_phase': 'new_GIN_aligned_warmstart_full_objective_every_epoch_reset_optimizer_epoch0',
...
def load_generator(config, summary, frozen_rules, bridge, device):
    ...
    warm = torch.load(config['warmstart_checkpoint'], map_location=device, weights_only=False)
    if warm.get('epoch_completed') != 35 or warm.get('run_kind') != 'FORMAL_REPAIR_FINETUNE':
        raise ValueError('EXPECTED_REAL_PAUSED_EPOCH35_CHECKPOINT')
    if warm.get('config_sha256') != config['warmstart_source_config_sha256']:
        raise ValueError('WARMSTART_SOURCE_CONTRACT_CHANGED')
...
def train_update(model, optimizer, fss, bridge, index, permutation):
    ...
    for position in permutation:
        ...
        if not torch.isfinite(loss): raise ValueError('nonfinite frozen GIN training NLL')
        # One optimizer update after the complete logical budget. This small
        # backward frees per-application GIN buffers while retaining the one
    ...
    if any(p.grad is not None for p in bridge.model.parameters()):
        raise ValueError('frozen GIN received parameter gradients')
    optimizer.step()
...
def real_oracle_identity_canary(model, fss, bridge, item):
    """Known legal train identity fixture through real generator and GIN.
    ...
    if not torch.allclose(probabilities.detach().cpu(),torch.tensor(expected['probabilities']),atol=2e-6,rtol=0):
        raise ValueError('real GIN identity-forward parity failed')
    ...
    if not finite or norm<=0 or any(p.grad is not None for p in bridge.model.parameters()):
        raise ValueError('real generator/GIN identity gradient failed')
    result={'state':'PASS','parent_id':row['id'],'canonical_smiles':product.canonical_smiles,
        ...
        'frozen_GIN_gradient':False,'fixture_kind':'synthetic_train_identity_not_generated_recourse',
...
def require_gpu_canary(config, config_sha):
    ...
    if (terminal.get('state')!='CANARY_COMPLETE' or terminal.get('optimizer_updates')!=2
            ...
            or identity.get('frozen_GIN_gradient') is not False or identity.get('target_flip_claimed') is not False):
...
def run_training(config_path, rematerialization_root, output_root, device, *, resume=False, canary=False,
    ...
    if terminal.get('state') != 'INPUTS_BOUND' or terminal.get('config_sha256') != config_sha:
        raise ValueError('GIN source rebinding and identity gate required')
    if not terminal.get('identity_fixtures') or terminal.get('positive_control', {}).get('state') != 'PASS':
        raise ValueError('real identity and positive legality control required')
    ...
    run_kind = 'ENGINEERING_CANARY' if canary else 'FORMAL_GIN_ALIGNED_WARMSTART'
    if resume:
    ...
    atomic_json(output/'execution_receipt.json', {'config_sha256': config_sha, 'pid': os.getpid(),
                'stage': 'GPU_TRAINING_CANARY' if canary else 'GIN_ALIGNED_GENERATOR_WARMSTART',
                ...
                'source_generator': config['warmstart_checkpoint'], 'source_generator_oracle': 'gine', 'training_oracle':'gin',
                'warmstart_epoch':35,'resume_old_trajectory_claimed':False, 'train_matches': len(train_index),
    ...
    if feasible and feasible['strict_flip_witness_found']:
        from src.baselines.bace_globalgce_aplus_export import export_pool
