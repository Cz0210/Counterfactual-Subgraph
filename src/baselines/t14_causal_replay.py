...
def validate_campaign(campaign_path: Path, *, source_spec_path: Path, output_root: Path) -> list[Path]:
    ...
    if value.get('authorization_id')!='T14_HEAD0_CAUSAL_REPLAY_170_V8_20260917':
        raise ValueError('EXISTING_V8_AUTHORIZATION_REQUIRED_NO_NEW_BUDGET')
...
def execute_replay(*, source_worktree, source_spec_path, output_root, gpu_uuid, campaign_path, diagnostic):
    ...
    with hold_t9_inputs(
        ...
        with full._bounded_t14_runtime(module=module, bridge=bridge, graph_store_path=store, seed=parameters.seed, expanded_capacity=full.TRANSITION_EXPANDED_CAPACITY, route_c_root=output_root / "route_c_state" if lowmemory else None, route_c_resume=lowmemory) as handles:
            ...
            def sink(row):
                pickle.dump(dict(step=250+started,evidence_kind='NEW_CAUSAL_REPLAY',**row),follower_calls,protocol=5)
                follower_calls.flush()
            with gzip.open(output_root / 'follower_calls.pkl.gz','wb') as follower_calls, gzip.open(output_root / "raw_step_observations.pkl.gz", "wb") as observations, (output_root / "step_summary.jsonl").open("x") as summaries, diagnostic.SamplingObserver(module,follower_code=native_move_code,action_lookup=handles.transition_map._entries.get,event_sink=sink) as observer:
                ...
                def on_step(loop_state):
                    ...
                    row['evidence_kind']='NEW_CAUSAL_REPLAY'
                    pickle.dump(row, observations, protocol=5)
                    observations.flush()
                    followers=[e for e in observer.events if e.get('api')=='follower_argmin']
                    if followers and any('one_edit' not in e or 'difference_bytes' not in e for e in followers):
                        raise ValueError('FIRST_FOLLOWER_CAPTURE_INCOMPLETE_STOP_BEFORE_NEXT_TRANSITION')
