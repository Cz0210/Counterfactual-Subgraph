...
class SamplingObserver:
    ...
    def __init__(self, module: Any, *, follower_code=None, action_lookup=None, event_sink=None):
        self.module = module
    ...
    def __enter__(self):
        ...
        def observed_argmin(*args, **kwargs):
            ...
            if self.capture_follower and frame.f_code is self._follower_target:
                if kwargs or len(args)!=1:raise ValueError('FOLLOWER_ARGMIN_CALL_CONTRACT_CHANGED')
                # Copy the actual consumed bytes BEFORE the original argmin.
                row=follower_snapshot(frame.f_locals,args[0],None)
            result = self._argmin(*args, **kwargs)
            if row is not None:
                finish_follower_snapshot(row,result)
                # Persist the consumed array/result even if later action/graph
                # binding fails. This sink is observational; no RNG or oracle.
                if self.event_sink is not None:self.event_sink(dict(row,capture_component='ARGMIN_CONSUMED'))
                if self.action_lookup is not None:
                    ...
                    # Only copy graph objects already present at the call site;
                    # no cache lookup, graph reconstruction or oracle invocation.
                    import copy
                    local=frame.f_locals
                    if 'start_map_data' not in local or 'target_graphs' not in local:
                        raise ValueError('FOLLOWER_ACTUAL_GRAPH_OBJECTS_MISSING')
                    row['before_graph']=copy.deepcopy(local['start_map_data'][row['head']][0])
                    row['after_graph']=copy.deepcopy(local['target_graphs'][row['selected_index']])
                    from src.baselines.comrecgc.graph_trace import collapse_semantic_transition_aliases
                    resolved=collapse_semantic_transition_aliases(row['before_graph'],row['after_graph'],
                        authoritative_historic_action=row['selected_action'],candidate_actions=[row['selected_action']],
                        source_head_id=row['head'])
                    row['one_edit']=resolved.audit_dict()
                    if resolved.excluded:raise ValueError('FOLLOWER_RECORDED_ACTION_NOT_ONE_EDIT')
                self.events.append(row)
                if self.event_sink is not None:self.event_sink(dict(row,capture_component='ACTION_GRAPH_BOUND'))
            return result
...
def follower_snapshot(local, argmin_input, selected):
    ...
    if len(ids)!=len(difference) or not len(ids):
        raise ValueError('FOLLOWER_CANDIDATE_ORDER_LENGTH')
    arrays={}
    for name in names:
    ...
    row=dict(api='follower_argmin',head=int(local['k']),source_hash=int(local['i']),
        lead_head=int(local['select']),candidate_order=ids,arrays=arrays,
        difference_bytes=difference.tobytes(order='C'),axis=None,flatten_order='C',
        nan_indices=np.flatnonzero(np.isnan(difference)).tolist(),
        inf_indices=np.flatnonzero(np.isinf(difference)).tolist(),
        duplicate_candidate_hashes=len(ids)-len(set(ids)),
        selection_mask=None,mask_scope='Native follower applies no additional mask; supplied candidates retain original order.',
        upstream_rejections='NOT_RECONSTRUCTED_AT_FOLLOWER_CALL',
        selection_rule='numpy.argmin axis=None; first minimum in C-order; NaN behavior remains NumPy native',
        capture_phase='PRE_CONSUMPTION_COPY',original_argmin_called_once=True,extra_rng_calls=0)
    if selected is not None:finish_follower_snapshot(row,selected)
    return row


def finish_follower_snapshot(row,selected):
    import numpy as np
    difference=row['arrays']['difference']['values']
    index=int(selected)
    if not 0<=index<len(difference):raise ValueError('FOLLOWER_ARGMIN_OUT_OF_BOUNDS')
    best=float(difference[index]);ordered=np.sort(difference,kind='stable')
    row.update(selected_index=index,selected_hash=row['candidate_order'][index],
        exact_minimum_indices=np.flatnonzero(difference==best).tolist(),min_value=best,
        second_min_value=float(ordered[1]) if len(ordered)>1 else None)
...
def follower_field_preflight(output_root):
    ...
    local=dict(k=0,i=11,select=3,target_graphs_hashes=list(range(100,140)),
    ...
    result=dict(state='FULL_FOLLOWER_RAW_SAVE_REOPEN_PASS',new_transitions=0,
        ...
        evidence_scope='SYNTHETIC_CAPTURE_SERIALIZATION_NOT_SCIENTIFIC_PARITY')
