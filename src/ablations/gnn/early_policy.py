...
def gpu_allowed(evidence, *, family):
    if family not in ("gnn", "llm", "ours_reach", "globalgce_aplus"):
    ...
    if family not in ("ours_reach", "globalgce_aplus") and evidence.get("gpu_idle_seconds", 0) < 1200:
    ...
    if family == "globalgce_aplus":
        if evidence.get("globalgce_aplus_contract_verified") is not True:
            blockers.append("GLOBALGCE_APLUS_TRAIN_CONTRACT_UNVERIFIED")
        if (evidence.get("gpu_index") != 0
            or (evidence.get("actual_gpu_observation", {}).get("process_count") != 0
                and not evidence.get("gpu_child_pid"))):
            blockers.append("GLOBALGCE_APLUS_REQUIRES_EXCLUSIVE_GPU0")
